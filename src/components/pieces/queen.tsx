...
export type QueenHandle = {
  playMoveAnimation: (from: [number, number, number], to: [number, number, number], onRest?: () => void) => void;
};

export const Queen = forwardRef<QueenHandle, PieceProps>(
  ({ color, position, onClick }, ref) => {
    const { nodes, materials } = useGLTF("/assets/ABeautifulGame.gltf") as any;
    const isWhite = color === "WHITE";
    const meshNode = isWhite ? nodes.Queen_W : nodes.Queen_B;
    const meshMaterial = isWhite ? materials.Queen_White : materials.Queen_Black;
