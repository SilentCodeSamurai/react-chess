...
export type BishopHandle = {
  playMoveAnimation: (from: [number, number, number], to: [number, number, number], onRest?: () => void) => void;
};

export const Bishop = forwardRef<BishopHandle, PieceProps>(
  ({ color, position, onClick }, ref) => {
    const { nodes, materials } = useGLTF("/assets/ABeautifulGame.gltf") as any;
    const isWhite = color === "WHITE";
    const meshNode = isWhite ? nodes.Bishop_W1 : nodes.Bishop_B1;
    const meshMaterial = isWhite ? materials.Bishop_White : materials.Bishop_Black;
